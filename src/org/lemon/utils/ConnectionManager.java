...
public class ConnectionManager {
	...
	public ConnectionManager(JComponent self, List<Pair> pair) {
		
		if(self == null || pair == null)
			return;
		
		Graphics2D g = (Graphics2D) self.getGraphics();
		g.setStroke(new BasicStroke(3));
		
		/*For each pair draw line or curve between them*/
		for(Pair p: pair) {
