...
public abstract class TransformFilter extends AbstractImageFilter {
    ...
    public LImage filter( LImage limage ) {
        ...
        if ( dst == null ) {
			...
			dstAsLi = new LImage( dst );
        }

		int[] inPixels = new int[ width * height ];
		inPixels = limage.getPixels(0, 0, width, height, inPixels);
		...
		for ( int y = 0; y < outHeight; y++ ) {
			...
			dstAsLi.setPixels( 0, y, transformedSpace.width, 1, outPixels );
		}
		return dstAsLi;
	}
	...
	protected LImage filterPixelsNN( BufferedImage dst, int width, int height, int[] inPixels, Rectangle transformedSpace ) {
		int srcWidth = width;
		int srcHeight = height;
		int outWidth = transformedSpace.width;
		int outHeight = transformedSpace.height;
		...
		for (int y = 0; y < outHeight; y++) {
			...
			outLi.setPixels( 0, y, transformedSpace.width, 1, outPixels );
		}
		return outLi;
	}
}
